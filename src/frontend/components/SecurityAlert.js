...
const SecurityAlert = () => {
  ...
  const checkSecurityIssues = () => {
    ...
    if (process.env.REACT_APP_PINATA_API_KEY === 'efe673ec8c032ecbd87a') {
      issues.push('Exposed Pinata API Key detected');
    }

    if (process.env.REACT_APP_STELLAR_ISSUER_PUBLIC_KEY === 'GAHDNV6A6NSOQM5AMU64NH2LOOAIK474NCGX2FXTXBKD5YUZLTZQKSPV') {
      issues.push('Exposed Stellar Public Key detected');
    }

    if (process.env.REACT_APP_INFURA_KEY === 'cd1f481035af45bd84d3b7589667d7e9') {
